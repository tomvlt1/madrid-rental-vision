...
unmodified — you can compare the two side-by-side.
...
### What this tells us
- Tabular alone gives R² 0.80 — size/zone/bedrooms do most of the work.
- **SigLIP adds +0.08 R²** on top of tabular (vs +0.05 for ResNet in v1).
- **Text becomes essentially redundant** once SigLIP is in the mix (0.882 → 0.884 is within noise). SigLIP was trained on image-text pairs, so it already encodes the semantic content the description was contributing.
...
1. **Listing-level dedup ran only on URL** in v1 — same property re-listed under a new ID could straddle train/test. v2's `clean_and_merge.py` dedups on `(price, sqft, rooms, bathrooms, location)` tuples too. ~46 re-listings collapsed in the original 1,425; ~567 collapsed in the new 4,708 net-new listings.
...
You'll need the v1 dataset (`data/processed/listings_clean.csv`) and the unified v2 dataset (not committed; build it from your own scrape). Then:
